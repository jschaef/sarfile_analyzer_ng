...
def single_multi(config_dict: dict, username: str):
    ...
    for file in sar_files:
        ...
        sel = st.checkbox(sar_files[sar_files.index(
            file)], key=key, value=has_clicked, on_change=delete_session_state_df_obj,
                args=([file]))
        if sel:
    ...
    if ph_show.checkbox('***show***',key='Show'):
        ...
        if sel_field:
            ...
            for sar_data in df_all:
                ...
                df_file = sar_data[0]
                file = sar_data[1]
                df = pl_helpers.get_data_frames_from__headers([selected], 
                    df_file, "header")[0]
                start = df['date'].min()
                end = df['date'].max()
            ...
            if aitem[selected] != 'CPU':
                ...
                for index in range(len(df_list_header)):
                    ...
                    file_name = path.basename(df_list_header[index][1])
                    df_list_header[index][0] = \
                        dia_compute.prepare_single_device_for_pandas(index_df,
                        start, end, sub_item, file_name)
            ...
            if pd_or_dia == 'Diagram':
                if chart_field:
                    ...
                    for item in range(len(chart_field)):
                        df_date = helpers.get_df_from_start_end(chart_field[item][0], 
                            start, end)
        ...
        for key in st.session_state:
            if key.endswith('_end') or key.endswith('_start') or \
                key.endswith('_obj'):
