...
def df_reset_date(df: pl.DataFrame, os_details: str, column_name: str, alias: str, \
        tformat: str='24') -> pl.DataFrame:
    date_str, format = format_date(os_details)
    if tformat == 'AM_PM':
        date_column = df.select(
            pl.col(column_name).str.extract(r'(^\d{2}:\d{2}:\d{2}\s+(AM|PM))'
                ).alias(alias)
        )
    else:
        date_column = df.select(
    ...
    date_column = df.select(
        pl.col(alias).str.replace(r'(^.*$)', f"{date_str} $1")
    )
    df = df.with_columns(date_column)
    ...
    if tformat != 'AM_PM':
        ...
        df = df.with_columns(
            ...
            pl.col('header').str.replace(r'(^(\s*AM|PM\s+)(.*))', "$3")
        )
    return df

def df_clean_data(df: pl.DataFrame, column_name: str,) -> pl.DataFrame:
    clean_header = get_unwanted_headers()
    for item in clean_header:
        df = df.filter(
            pl.col(column_name) != item
        )
    return df

def replace_comma_with_point(df: pl.DataFrame, column_name: str) -> pl.DataFrame:
    df = df.with_columns(
        pl.col(column_name).str.replace_all(r'(,)', '.')
    )
    return df
...
def clean_header(df: pl.DataFrame, column_name: str) -> pl.DataFrame:
    ...
    for item in clean_header:
        df = df.with_columns(
            pl.col(column_name).str.replace(rf"^{item}\s*", "")
        )
    return df   

def df_clean_spaces(df: pl.DataFrame, column_name: str) -> pl.DataFrame:
    df = df.with_columns(
        pl.col(column_name).str.replace(r'\s+', ' ')
...
def get_metrics_from_df(df: pl.DataFrame, header: str, alias: str) -> list:
    ...
    df = df.with_columns(
        pl.col(header)
        .list.eval(pl.element().cast(pl.Float32, strict=False).drop_nulls())
        )
    return df
...
def get_df_from_sub_device(df: pl.DataFrame, column: str, sub_device: 
        str) -> pl.DataFrame:
...
def get_date_df(df: pl.DataFrame, column: str, start: pl.datetime,
    end: pl.datetime) -> pl.DataFrame:
...
def get_os_details_from_df(df: pl.DataFrame) -> str:
    return df.filter(pl.col("os_details").str.contains('Linux')
        )['os_details'].to_list()[0]

def get_restart_headers(df: pl.DataFrame) -> list:
    if column_exists(df, "restart"):
        return df.filter(pl.col("restart").str.contains('RESTART')
            )['restart'].to_list()
    else:
        return []
...
def get_data_frames_from__headers(headers: list, df: pl.DataFrame, 
        column: str) -> list:
    df_list = []
...
def get_complete_dataframe_from_headers(headers: list, df: pl.DataFrame, 
