...
def get_data_frame(file_name, user_name):
    ...
    if rs:
        ...
        parquet_obj = redis_mng.get_redis_val(
            r_item, property=file_name_parquet)
        parquet_mem = io.BytesIO(parquet_obj)
        try:
            df = pl.read_parquet(parquet_mem)
            print(f'{r_item}, {file_name_parquet} \
            ...
            try:
                ...
                redis_mng.set_redis_key(parquet_obj, r_item, property=file_name_parquet,
                    )
                print(f'{r_item}, {file_name_parquet} saved to redis at {datetime.now().strftime("%m/%d/%y %H:%M:%S")}')
...
def parse_sar_file(file_path: str, username:str, DEBUG: bool=False) -> pl.DataFrame:
    ...
    if length_restart_field > 0:
        ...
        restart_col = pl.Series("Linux_Restart", [""] * (len(df) - 
            length_restart_field))
    ...
    if rs:
        ...
        if not p_obj:
            try:
                ...
                redis_mng.set_redis_key(mem_obj, r_item, 
                    property=file_name_parquet)
                print(
                    f'{r_item}, {file_name_parquet} saved to redis at {datetime.now().strftime("%m/%d/%y %H:%M:%S")}')
            except Exception as e:
                print(f'could not connect to redis server or save {file_name_parquet} to redis server',)
                print(f'exception is {e}')
