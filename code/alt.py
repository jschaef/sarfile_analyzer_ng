...
def create_reboot_rule(df, property, restart_headers, os_details, col=None, 
        col_value=None):
    ...
    for header in restart_headers:
        ...
        rule = alt.Chart(mdf).mark_rule(color='red').encode(
             x=alt.X('utchoursminutes(x)', type='temporal', 
...
def overview_v1(df, restart_headers, os_details, font_size=None, width=None,
...
def overview_v3(collect_field, reboot_headers, width, height, lsel, font_size,
    ...
    if z_fields:
        ...
        while z_fields:
            ...
            z_field = t_field[0]
            filename = t_field[1]
            reboot_text = return_reboot_text(z_field, y_pos, col=color_item,
                col_value=filename)
            reboot_text = reboot_text.encode(
                opacity=alt.condition(selection, alt.value(1), alt.value(0)),
                color=color_x
